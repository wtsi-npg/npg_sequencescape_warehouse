...
{
  lives_ok {
       $loader  = npg_warehouse::loader->new( lims_driver_type => 'xml',
...

{
  ...
  while (my $row = $rs->next) {
    is($row->manual_qc, ($row->position > 4) ? 1 : undef,
      'manual qc outcome for run with "data discarded" status');
  }
}
...

{
  ...
  is_deeply($loaded, $expected, 'run meta info loaded for runs 4025 and 4799');
  ...
  is_deeply($loaded, $plexes, 'run meta info loaded for runs 4799 position 7 plexex 1-4');
  is_deeply($tag_info, $expected_tag_info, 'tag info loaded for runs 4799 position 7 plexex 1-4');
...
{
  my $loader1   = npg_warehouse::loader->new( lims_driver_type => 'xml',
                                              _autoqc_store => $autoqc_store,
                                              _schema_npg   => $schema_npg, 
                                              _schema_qc    => $schema_qc, 
                                              _schema_wh    => $schema_wh,
